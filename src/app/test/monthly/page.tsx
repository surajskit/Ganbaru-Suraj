...
async function fetchMonthlyMcqs(): Promise<Mcq[]> {
  try {
    const res = await fetch(`${API_BASE}/api/test/monthly`, {
      cache: "no-store",
    });
...
export default async function MonthlyTestPage() {
  const mcqs = await fetchMonthlyMcqs();
    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-5 grid grid-cols-1 lg:grid-cols-[260px_1fr_320px] gap-4 items-start">
        ...
        <div className="space-y-4">
          <TestQuiz
            title="Monthly Test"
            description="Questions from this month."
            questions={mcqs}
          />
        </div>
