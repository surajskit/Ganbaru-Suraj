...
async function fetchDailyMcqs(date?: string): Promise<Mcq[]> {
  const query = date ? `?date=${encodeURIComponent(date)}` : "";

  try {
    const res = await fetch(`${API_BASE}/api/test/daily${query}`, {
      cache: "no-store",
    });
...
export default async function DailyTestPage({
  ...
  const mcqs = await fetchDailyMcqs(sp.date);
    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-5 grid grid-cols-1 lg:grid-cols-[260px_1fr_320px] gap-4 items-start">
        ...
        <div className="space-y-4">
          <TestQuiz
            title="Daily Test"
            description={sp.date ? `Date: ${sp.date}` : "Questions for today."}
            questions={mcqs}
          />
        </div>
